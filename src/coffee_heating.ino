...
#define CENTIGRADES 100


#define VOLTAGE_TO_CELSIUS(x) (x * (FIVE_VOLTS / AD_RESOLUTION) * CENTIGRADES)

// declaração variáveis globais:
bool ledState = 0;            // para fazer variar nível lógico led monitor
const byte LED_MONITOR = 13;  // toggles a cada 0,5 segundos monitorando algo controle (ISR)
const byte TIP120 = 7;        // Dispositivo a ser controlado
const byte BUZZER = 6;       // Buzzer, feedback adutivo comandos recebidos pela placa
const byte LM35 = A0;         // pino (A/D) que recebe sinal analógico
...
void setup() {
  ...
  analogWrite(TIP120, 0);  // garante que este led inicia apagado
...
ISR(TIMER2_COMPA_vect) {  //timer2 interrupt @ 100 Hz
  ...
  pv = analogRead(LM35);  // y[k] = info do sensor (A/D de 12-bits: [0..4095])
  y = VOLTAGE_TO_CELSIUS(pv);  // y[k] = info do sensor (em graus Celsius)
  ...
  limita_valor(&mv);
  analogWrite(TIP120, mv);  // valores entre 0 ~ 255 (0 ~ 100%) de dutty-cycle
